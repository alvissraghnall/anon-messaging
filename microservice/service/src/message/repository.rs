...
mod tests {
    ...
    async fn test_insert_message() {
        ...
        let result = mock.insert_message(sender_id, recipient_id, encrypted_content, signature, parent_id)
            .await
            .unwrap();
    ...
    async fn test_insert_message_failure() {
        ...
        let result = mock.insert_message(
            sender_id,
            recipient_id,
            "encrypted_content",
            None,
    ...
    async fn test_get_conversation() {
        ...
        let result = mock.get_conversation(user1_id, user2_id, limit).await.unwrap();
    ...
    async fn test_get_thread_replies() {
        ...
        let result = mock.get_thread_replies(parent_id, limit, offset).await.unwrap();
    ...
    async fn test_get_complete_thread() {
        ...
        mock.expect_get_complete_thread()
            .with(
                predicate::eq(thread_root_id),
                predicate::eq(limit)
            )
            .times(1)
            .returning(move |_, _| Ok(thread.clone()));
        
        let result = mock.get_complete_thread(thread_root_id, limit).await.unwrap();
