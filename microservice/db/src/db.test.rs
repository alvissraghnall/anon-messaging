...
async fn test_get_user_by_pubkey_found() {
    ...
    let user = insert_user(&pool, &public_key_hash, &public_key, username)
        .await
        .unwrap();
...
async fn test_get_user_by_pubkey_not_found() {
    let pool = setup_test_db().await;
    let (_, public_key_hash) = generate_key().await;
...
async fn test_fetch_public_key_hash_success() {
    ...
    assert_eq!(&public_key_hash, original_user.public_key_hash.as_str());
}
...
async fn test_update_user_username() {
    ...
    println!("{} {}", updated_user.updated_at, original_user.updated_at);
    assert!(updated_user.updated_at > original_user.updated_at);
}
...
async fn test_update_user_with_empty_values() {
    ...
    let result = update_user(&pool, user_id, Some(empty_username), None, None).await;
