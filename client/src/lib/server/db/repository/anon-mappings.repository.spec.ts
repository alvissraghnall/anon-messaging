...
beforeEach(async () => {
  ...
  await db.insertInto('users').values({ id: 'user-123', username: 'asap', password: 'rocky' }).execute();
});
...
describe('AnonMappingRepository', () => {
  ...
  test('create mapping with invalid user_id fails', async () => {
    ...
    await expect(repo.createMapping(invalidMapping)).rejects.toThrowError('FOREIGN KEY constraint failed');
  });
